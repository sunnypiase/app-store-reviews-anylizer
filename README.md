...
- **Sentiment**: classified by the Gemini API (`gemini-flash-lite-latest`,
  chosen for its 15-requests/minute free tier — see
  `docs/SENTIMENT_ANALYSIS_RESULTS.md` for why: macro F1 0.87 vs. 0.55 for
  the lexicon-based alternative below), reading only each review's
  `title`/`content`, never its star rating. Requires `GEMINI_API_KEY` (see
  `.env.example`) — get a free one at
...
- **Negative-review keywords**: reviews with a rating ≤ 2 are "negative
  ...
  *whole* sample so ubiquitous terms score low, then evaluated only on the
  ...
  each — a deliberately simple, rule-based step, independent of the
  sentiment classifier above.
...
- Gemini's free tier is capped at 15 requests/minute; a sample large enough
  to need several concurrent batches of 150 reviews could occasionally hit
  that cap and fall back to VADER for that request.
